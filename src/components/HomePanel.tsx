  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isSignUp) {
        ...
        // Log in / Register with input email
        ...
        // Let's check if the email belongs to a pre-set demo account to maintain consistency
        let finalRole: UserRole = "donor";
        if (loginEmail.toLowerCase().includes("ngo") || loginEmail.toLowerCase().includes("receiver")) {
          finalRole = "receiver";
        }
        
        const profile = await FoodLinkService.saveUserProfile(tempUid, formattedName, loginEmail, finalRole);
        onLogin(profile);
      }
    } catch (e: any) {
